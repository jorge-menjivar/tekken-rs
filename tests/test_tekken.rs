// token ids and vocab sizes are copied verbatim from mistral-common output; separators would make them harder to compare (similar_names: tokens_bos/tokens_eos are the clearest names for these fixtures)
...
fn test_empty_and_whitespace_inputs() {
    ...
    for input in whitespace_cases {
        ...
        let decoded = tokenizer
            ...
            .unwrap();
        assert_eq!(input, decoded, "Whitespace handling failed for: {input:?}");
    }
}
...
fn test_single_characters() {
    ...
    for ch in b'a'..=b'z' {
        ...
        let decoded = tokenizer
            .decode(&tokens, SpecialTokenPolicy::Ignore)
            .unwrap();
        assert_eq!(input, decoded, "Single character test failed for: {input}");
    }
...
fn test_special_characters_comprehensive() {
    ...
    for chars in special_chars {
        ...
        assert_eq!(
            chars, decoded,
            "Special characters test failed for: {chars}"
        );
    }
}
