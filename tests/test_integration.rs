...
fn test_no_tools_conversation_structure() {
    ...
    for (role, content) in messages.iter() {
        ...
        assert!(
            tokens.len() > 0,
            "Should produce at least one token for: {}",
            content
        );
        assert!(
            tokens.len() < content.len() + 10,
            "Token count seems unreasonable for: {}",
            content
        );
    }
}
...
fn test_mathematical_expressions() {
    ...
    for expr in math_expressions.iter() {
...
fn test_code_examples() {
    ...
    for code in code_examples.iter() {
...
fn test_multilingual_content() {
    ...
    for text in multilingual_examples.iter() {
...
fn test_token_efficiency() {
    ...
    for (text, max_expected_tokens) in efficiency_tests.iter() {
        let tokens = tokenizer.encode(text, false, false).unwrap();
        assert!(
...
fn test_special_token_injection_safety() {
    ...
    for input in potentially_problematic_inputs.iter() {
        ...
        assert_eq!(
            *input, decoded,
            "Special token string handling failed: {}",
            input
        );
        ...
        assert_eq!(
            *input, decoded_with_special,
            "BOS/EOS + special token strings failed: {}",
            input
        );
    }
}
