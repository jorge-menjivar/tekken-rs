        ...
        for &token in tokens.iter() {
            ...
            if is_byte {
                ...
                assert!(
                    ...
                    "Keep policy should succeed for byte token {}",
                    token
                );
                assert!(
                    byte_piece_ignore.is_ok(),
                    "Ignore policy should succeed for byte token {}",
                    token
                );
...
fn test_id_to_piece_functionality() {
    ...
    assert!(
        ...
        "BOS piece should look like a BOS token: {}",
        bos_piece
    );
    ...
    assert!(
        ...
        "EOS piece should look like an EOS token: {}",
        eos_piece
    );
}
...
fn test_get_control_token() {
    ...
    for (token_str, should_exist) in test_tokens.iter() {
...
fn test_is_special_token_method() {
    ...
    for &token in tokens.iter() {
        assert!(
            ...
            "Regular text token {} should not be special",
            token
        );
    }
