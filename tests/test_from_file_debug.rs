...
fn test_from_file_debug() {
    ...
    let content = std::fs::read_to_string("tekken.json").expect("Failed to read tekken.json file");
    println!(
    ...
    let tokenizer =
        Tekkenizer::from_file("tekken.json").expect("Failed to create tokenizer from file");
